...
if fv.UlLogPPS != 0 || fv.AvgLen != 0 || fv.NewFlowRate != 0 || fv.FanOut != 0 {
t.Fatalf("expected zero vector when PacketCount==0, got %+v", fv)
}
// Check downlink features are also zero
if fv.DlLogPPS != 0 || fv.DlAvgLen != 0 || fv.PPSRatio != 0 || fv.ByteRatio != 0 || fv.AckRatio != 0 {
...
if fv.UlLogPPS <= 0 {
t.Fatalf("expected UlLogPPS > 0, got %v", fv.UlLogPPS)
}
...
// DlLogPPS should be log10(2000) ≈ 3.3010
expectedDlLogPPS := math.Log10(2000)
if math.Abs(fv.DlLogPPS-expectedDlLogPPS) > 0.001 {
t.Fatalf("expected DlLogPPS=%.4f, got %v", expectedDlLogPPS, fv.DlLogPPS)
}
