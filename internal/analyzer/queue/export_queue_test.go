...
func TestExportMessage_InferenceResult(t *testing.T) {
	...
	if converted.Supi != result.Supi {
		t.Errorf("Expected SUPI %s, got %s", result.Supi, converted.Supi)
	}

	if converted.AnomalyScore != result.AnomalyScore {
		t.Errorf("Expected anomaly score %.3f, got %.3f", result.AnomalyScore, converted.AnomalyScore)
	}
}
